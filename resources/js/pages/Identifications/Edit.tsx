...
import { update as eventsIdentificationUpdate } from '@/routes/events/identification';
...
export default function Edit({ event, identification }: Props) {
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        main_category: identification.main_category || '',
        sub_category: identification.sub_category || '',
        assumptions: {
            what_assumptions: identification.assumptions?.what_assumptions || '',
            ignored_information: identification.assumptions?.ignored_information || '',
            protected_beliefs: identification.assumptions?.protected_beliefs || '',
        },
        pattern_recognition: {
            noticed_before: identification.pattern_recognition?.noticed_before || '',
            triggers: identification.pattern_recognition?.triggers || '',
            personal_or_organizational: identification.pattern_recognition?.personal_or_organizational || '',
            common_thread: identification.pattern_recognition?.common_thread || '',
        },
    });

    // Initialize subcategories on mount if main_category exists
    useEffect(() => {
        if (identification.main_category) {
            setAvailableSubcategories(getSubcategoriesForMain(identification.main_category as MainCategory));
        }
    }, [identification.main_category]);
